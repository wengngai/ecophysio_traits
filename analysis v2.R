...
PCA.traits <- rda(traits_sp[sel], scale = T)
...
# PC3: hydraulic investment
# PC4: hydraulic safety(-ve)-efficiency(+ve) tradeoff

# invert so that higher tPC3 = more hydraulic investment
PCA.traits$CA$u[,3] <- -1 * PCA.traits$CA$u[,3]
PCA.traits$CA$v[,3] <- -1 * PCA.traits$CA$v[,3]
...
### SSI correlations with trait PCs
summary(best.dssi)
inv.logit <- function(y) exp(y) / (1 + exp(y))

# predictions
newtPC1 <- seq(-0.5, 0.5, 0.01)
dpred1 <- predict(best.dssi, newdata = data.frame(tPC1 = newtPC1, tPC2 = 0, tPC3 = 0, tPC4 = 0), se.fit = T)
newtPC2 <- seq(-0.5, 0.5, 0.01)
dpred2 <- predict(best.dssi, newdata = data.frame(tPC2 = newtPC2, tPC1 = 0, tPC4 = 0, tPC3 = 0), se.fit = T)
newtPC3 <- seq(-0.3, 0.6, 0.01)
dpred3 <- predict(best.dssi, newdata = data.frame(tPC3 = newtPC3, tPC1 = 0, tPC2 = 0, tPC4 = 0), se.fit = T)
newtPC4 <- seq(-0.5, 0.5, 0.01)
dpred4 <- predict(best.dssi, newdata = data.frame(tPC4 = newtPC4, tPC1 = 0, tPC2 = 0, tPC3 = 0), se.fit = T)

#pdf("D:\\Dropbox\\Functional Traits Project\\Figures\\SSI correlations.pdf", height=7, width=4)
par(mfrow = c(2, 1), mar = c(6,7,1,2), mgp = c(4,1,0))
# a) PC1
plot(traits_sp$SSI ~ tPC1, type="n", xlim = c(-0.35, 0.4),
     ylab = "", xlab = "Trait PC1\n(Photosynthesis investments)",
     cex.lab = 1.4)
...
text(traits_sp$SSI ~ tPC1, labels=traits_sp$Species, 
     cex = ifelse(traits_sp$Species %in% c("MBE", "TWA"), 1.2, 0.8),
     col = ifelse(traits_sp$Species %in% c("MBE", "TWA"), "black", "grey60"))
mtext(side = 3, text = " a)", cex = 1.2, line = -1.5, adj = 0)
# b) PC2
plot(traits_sp$SSI ~ tPC2, type="n", xlim = c(-0.42, 0.4),
     ylab = "", xlab = "Trait PC2\n(?? dimension)",
     cex.lab = 1.4)
polygon(with(dpred2, inv.logit(c(fit + se.fit, rev(fit - se.fit)))) ~ c(newtPC2,rev(newtPC2)), border = F, col = "#66D7D16e")
lines(inv.logit(dpred2$fit) ~ newtPC2, lwd = 4, col = "white")
text(traits_sp$SSI ~ tPC2, labels=traits_sp$Species, 
...
# a) PC2
plot(traits_sp$SSI ~ tPC2, type="n", xlim = c(-0.35, 0.4),
     ylab = "", xlab = "Trait PC2\n(Twig hydraulic and wood investments)",
     cex.lab = 1.4)
polygon(with(dpred2, inv.logit(c(fit + se.fit, rev(fit - se.fit)))) ~ c(newtPC2,rev(newtPC2)), border = F, col = "#66D7D16e")
lines(inv.logit(dpred2$fit) ~ newtPC2, lwd = 4, col = "white")
text(traits_sp$SSI ~ tPC2, labels=traits_sp$Species, 
     cex = ifelse(traits_sp$Species %in% c("ASY", "PPI"), 1.2, 0.8),
     col = ifelse(traits_sp$Species %in% c("ASY", "PPI"), "black", "grey60"))
mtext(side = 3, text = " a)", cex = 1.2, line = -1.5, adj = 0)

# b) PC4
plot(traits_sp$SSI ~ tPC4, type="n", xlim = c(-0.42, 0.4),
     ylab = "", xlab = "Trait PC4\n(?? dimension)",
     cex.lab = 1.4)
polygon(with(dpred4, inv.logit(c(fit + se.fit, rev(fit - se.fit)))) ~ c(newtPC4,rev(newtPC4)), border = F, col = "#66D7D16e")
lines(inv.logit(dpred4$fit) ~ newtPC4, lwd = 4, col = "white")
text(traits_sp$SSI ~ tPC4, labels=traits_sp$Species, 
     cex = ifelse(traits_sp$Species %in% c("PAX", "ACL"), 1.2, 0.8),
     col = ifelse(traits_sp$Species %in% c("PAX", "ACL"), "black", "grey60"))
mtext(side = 3, text = " b)", cex = 1.2, line = -1.5, adj = 0)

# c) PC7
plot(traits_sp$SSI ~ tPC7, type="n", xlim = c(-0.3, 0.4),
     ylab = "", xlab = "Trait PC7\n(Twig hydraulic and wood investments)",
     cex.lab = 1.4)
polygon(with(dpred7, inv.logit(c(fit + se.fit, rev(fit - se.fit)))) ~ c(newtPC7,rev(newtPC7)), border = F, col = "#66D7D16e")
lines(inv.logit(dpred7$fit) ~ newtPC7, lwd = 4, col = "white")
text(traits_sp$SSI ~ tPC7, labels=traits_sp$Species, 
...
legend <- c("Stomatal traits",
  "Leaf anatomical traits",
  "Leaf economic spectrum traits",
  "Twig anatomical traits",
  "Wood traits")

#pdf("D:\\Dropbox\\Functional Traits Project\\Figures\\Trait PCA v2.pdf", height=12, width=6)
#jpeg("D:\\Dropbox\\Functional Traits Project\\Figures\\Trait PCA v2.jpg", height=12, width=6, units="in", res=300)

# a) PC1 - PC2
par(mfrow = c(2,1), mar = c(5,5.5,2,2))
plot(x = PCA.traits$CA$v[,1]*1.25, y = PCA.traits$CA$v[,2]*1.25, type = "n",
     ylab = "Trait PC2 (17.9% variance explained)", xlab = "Trait PC1 (23.5% variance explained)",
     cex.lab = 1.5)
...
plot(x = PCA.traits$CA$v[,3]*1.25, y = PCA.traits$CA$v[,4]*1.25, type = "n",
     ylab = "Trait PC4 (9.8% variance explained)", xlab = "Trait PC3 (16.3% variance explained)",
     cex.lab = 1.5, ylim = c(-0.8, 0.5))
for(i in 1:nrow(PCA.traits$CA$v)){
  arrowcol <- col.vec[i]
...
legend('bottomleft', ncol = 2, legend = legend, lwd = 2, col = cols, bty = "n", cex = 0.8)
dev.off()
...
names(traits_sp)
plot(b ~ Th_SM, data = traits_sp, type = "n")
text(b ~ Th_SM, data = traits_sp, labels = Species)

plot(Th_PM ~ Th_SM, data = traits_sp, type = "n")
text(Th_PM ~ Th_SM, data = traits_sp, labels = Species)

plot(a ~ Th_SM, data = traits_sp, type = "n")
text(a ~ Th_SM, data = traits_sp, labels = Species)

plot(a ~ VA, data = traits_sp, type = "n")
text(a ~ VA, data = traits_sp, labels = Species)

plot(a ~ T_VD, data = traits_sp, type = "n")
text(a ~ T_VD, data = traits_sp, labels = Species)
